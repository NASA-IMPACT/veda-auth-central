...
package com.veda.central.service.credential.store;
...
import com.veda.central.core.credential.store.api.GetOperationsMetadataRequest;
import com.veda.central.core.credential.store.api.GetOperationsMetadataResponse;
import com.veda.central.core.credential.store.api.GetOwnerIdResponse;
import com.veda.central.core.credential.store.api.OperationMetadata;
import com.veda.central.core.credential.store.api.OperationStatus;
...
import com.veda.central.core.model.credential.store.CredentialEntity;
import com.veda.central.core.repo.credential.store.CredentialRepository;
...
public class CredentialStoreService {
        ...
        try {
            ...
            if (response != null && response.getData() != null && response.getData().getId() != null) {
                statusUpdater.updateStatus(Operations.PUT_CREDENTIAL.name(),
                        com.veda.central.core.model.commons.OperationStatus.SUCCESS,
                        request.getOwnerId(),
                        null);
                ...
                statusUpdater.updateStatus(Operations.PUT_CREDENTIAL.name(),
                        com.veda.central.core.model.commons.OperationStatus.FAILED,
                        request.getOwnerId(),
                        null);
    ...
    public CredentialMetadata getNewVedaCredential(GetNewVedaCredentialRequest request) {
        try {
            ...
            VaultResponseSupport<Credential> response = vaultTemplate.read(path, Credential.class);
            if (response == null || response.getData() == null || response.getData().getId() == null) {
                String msg = "Get new veda operation failed for " + request.getOwnerId();
                LOGGER.error(msg);
                statusUpdater.updateStatus(Operations.GENERATE_VEDA_CREDENTIAL.name(),
                        com.veda.central.core.model.commons.OperationStatus.FAILED,
                        request.getOwnerId(),
                        null);
    ...
    public CredentialMetadata getVedaCredentialFromToken(TokenRequest request) {
        try {
            ...
            String msg = "Error while extracting token from the VEDA credentials ";
            LOGGER.error(msg);
            throw new InternalServerException(msg, ex);
        }
    }

    public CredentialMetadata getVedaCredentialFromClientId(GetCredentialRequest request) {
        try {
    ...
    public GetAllCredentialsResponse getAllCredentialsFromToken(TokenRequest request) {
        try {
            ...
            String validatingPath = BASE_PATH + entity.getOwnerId() + "/" + Type.VEDA.name();
            ...
            if (paths != null && !paths.isEmpty()) {
                for (String key : paths) {
                    ...
                    if (key.equals(Type.VEDA.name())) {
                        ...
                        credentialsBuilder.setVedaClientId(crRe.getData().getId())
                                .setVedaClientSecret(crRe.getData().getSecret())
                                .setVedaClientIdIssuedAt(entity.getIssuedAt().getTime())
                                .setVedaClientSecretExpiredAt(entity.getClientSecretExpiredAt());
